 ...
 * Imports
 */
import { addOneListener, addListener, mainFrame } from './common.js';
...
let Local = {
    ...
    reloadFullPage: function (processesData) {
        ...
        // default table 
        loadProcessesTable(processesData);

        if (dateNow.getDate() < 10) {
            gTimeLineStart = new Date(dateNow.getFullYear(), dateNow.getMonth() - 1, 1);
            gTimeLineLength = 3;
            addMonthBefor(gTimeLineStart, processesData);
        } else {
        ...
        addMonthAfter(gTimeLineEnd, processesData);
        //addSubProject(111, processesData, gTimeLineStart, gTimeLineLength);
        //addSubProject(113, processesData, gTimeLineStart, gTimeLineLength);
        addListener('process-names-box-item', 'click', function (fullId) {
            let isOpened = false;
            ...
            if (isOpened) {
                ...
                Local.addSubProject(id, processesData, gTimeLineStart, gTimeLineLength);
    ...
    addSubProject: function (parentId, processesData, firthMonth, numOfMonth) {
        ...
        for (var index = 0; index < processesData.length; index++) {
            ...
            let processNamesBoxItem = document.createElement("div");
            processNamesBoxItem.className = "process-names-box-item display-flex process_" + parentId;
            ...
            let tasksTableTr4 = document.createElement("tr");
            tasksTableTr4.id = "process_row_" + process.ProjectId;
            tasksTableTr4.className = "tasksTableTr process_" + parentId;
            tasksTableTr4.style = "height: " + tasksTableTdWidth + "px;";
        ...
        addListener('process-names-box-item', 'click', function (fullId) {
            Local.addSubProject(fullId.split('_')[1], processesDataArray, gTimeLineStart, gTimeLineLength);
        });
    },
